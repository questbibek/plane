...
> **Migrating from the current stack:** your data lives in Docker volumes
> (`pgdata`, `uploads`, …). If the new compose project name differs, the volumes
> differ too. To keep existing data, run this compose from the **same project
> dir/name** as the current one, or `docker volume` rename. Greenfield (per the
> plan) = just bring the old stack down (`docker compose down`) and the new one up.
